...
// Resend an issue protocol to the customer (S-05) — the recovery path that makes
// `email_deliveries` worth having. Reached from the post-submit `email` overlay
// and from the dispatch row's badge, both of which exist because a customer with
// no account and no portal has email as their only channel.
//
...
// object already sits in storage, which is exactly why `create_protocol` stores
// `pdf_path` at all.
//

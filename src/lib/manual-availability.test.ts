...
describe("resolveAvailability", () => {
  it("is idle until vehicle, pickup and return are all present", () => {
    expect(resolveAvailability("", "2032-04-01", "2032-04-05", NONE, "ready").state).toBe("idle");
    expect(resolveAvailability(VEHICLE, "", "2032-04-05", NONE, "ready").state).toBe("idle");
    expect(resolveAvailability(VEHICLE, "2032-04-01", "", NONE, "ready").state).toBe("idle");
  });

  it("is idle even while the ranges are loading — there is nothing to resolve yet", () => {
    expect(resolveAvailability("", "2032-04-01", "2032-04-05", NONE, "loading").state).toBe("idle");
  });

  it("is invalid with a message when the return precedes the pickup", () => {
    const result = resolveAvailability(VEHICLE, "2032-04-10", "2032-04-05", NONE, "ready");
  ...
  it("is invalid for a same-day range (the half-open window needs start < end)", () => {
    expect(resolveAvailability(VEHICLE, "2032-04-01", "2032-04-01", NONE, "ready").state).toBe("invalid");
  });

  it("is invalid for a pickup in the past", () => {
    expect(resolveAvailability(VEHICLE, "2020-01-01", "2020-01-05", NONE, "ready").state).toBe("invalid");
  });

  it("invalid beats a loading fetch — the range is wrong whatever the vehicle is booked for", () => {
    expect(resolveAvailability(VEHICLE, "2032-04-10", "2032-04-05", NONE, "loading").state).toBe("invalid");
  });

  it("is error when the ranges read failed, and error beats a would-be available answer", () => {
    const result = resolveAvailability(VEHICLE, "2032-04-01", "2032-04-05", NONE, "error");
  ...
  it("is checking while the read is in flight", () => {
    expect(resolveAvailability(VEHICLE, "2032-04-01", "2032-04-05", NONE, "loading").state).toBe("checking");
  });

  it("is available for a free range once the ranges are ready", () => {
    expect(resolveAvailability(VEHICLE, "2032-04-01", "2032-04-05", NONE, "ready").state).toBe("available");
    expect(resolveAvailability(VEHICLE, "2032-04-01", "2032-04-05", BUSY, "ready").state).toBe("available");
  });

  it("is conflict for a range that collides with a busy range", () => {
    // Spans the booking's interior.
    expect(resolveAvailability(VEHICLE, "2032-04-11", "2032-04-13", BUSY, "ready").state).toBe("conflict");
    // Ends on the booking's return day — morning taken (a `pickupOnly` day).
    expect(resolveAvailability(VEHICLE, "2032-04-08", "2032-04-15", BUSY, "ready").state).toBe("conflict");
  });
  ...
  it("accepts the half-day changeovers the EXCLUDE window leaves free", () => {
    ...
    expect(resolveAvailability(VEHICLE, "2032-04-06", "2032-04-10", BUSY, "ready").state).toBe("available");
    // Starts on the booking's RETURN day: the afternoon is still free.
    expect(resolveAvailability(VEHICLE, "2032-04-15", "2032-04-18", BUSY, "ready").state).toBe("available");
  });
});

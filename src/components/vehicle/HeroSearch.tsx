...
import { categoryLabel, vehicle } from "../../lib/i18n/vehicle";

// The landing hero's primary action (design screen 07): a Type / Dates / Branch
...
// Branch is a single fixed location (no location concept in the schema yet), so
// it renders as a static field, not a control. Date validation mirrors the
// catalog's rule via `validateDateRange`.
//
// The one ISLAND on the landing page, so it reaches its copy through the
// per-namespace `translator` rather than the composed `Astro.locals.t` — the
...
export default function HeroSearch({ category = null, locale }: Props) {
  const t = translator(locale, vehicle);
      ...
      <div className="bg-card flex flex-col rounded-[22px] p-3 [box-shadow:0_18px_40px_-14px_rgba(0,0,0,0.30)] xl:flex-row xl:items-center xl:p-[7px] xl:[box-shadow:0_18px_40px_-12px_rgba(0,0,0,0.40)]">
        ...
        <div className="border-border min-w-0 flex-1 border-b px-4 py-2.5 xl:border-b-0 xl:py-2">
          ...
          <Select value={type} onValueChange={setType}>
            ...
            <SelectContent>
              <SelectItem value="all">{t("searchTypeAll")}</SelectItem>
              {CATEGORIES.map((c) => (
        ...
        {/* Branch — single fixed location (no location data model yet). The place
            name itself is not copy and stays verbatim in both locales. */}
        <div className="border-border xl:border-border min-w-0 flex-1 border-b px-4 py-2.5 xl:border-b-0 xl:border-l xl:py-2">
          <div className="text-muted-foreground text-[10px] font-bold tracking-[0.8px] uppercase">
            {t("searchBranch")}
          </div>

...
- [x] 7.5 Arrow keys move DOM focus across the grid and Enter selects the day the arrows landed on (F1) (focus movement driven: activeElement data-day 2026-08-23 → 2026-08-24 on ArrowRight, where before the fix DOM focus never left the first cell. Enter-selects-the-landed-on-day confirmed by the user 2026-08-21)
- [x] 7.6 The first click leaves the panel `idle` and the trigger showing the anchor date alone (D18's half-made-range state) — no return-before-pickup error between the two clicks (F2) (ROW CORRECTED 2026-08-21: it originally expected the trigger on **Wybierz termin**, which contradicts D18 — that string belongs to the NOTHING-picked state, while a pickup with no return shows the one date it has. Driven: one click on 2026-08-23 → trigger "23 sie 2026", panelIdle=true, invalidErrorShown=false. Before `min={1}` the first click returned {from:d, to:d}, which validateDateRange rejected, flashing "Data zwrotu musi być późniejsza niż data odbioru." between the two clicks)
- [x] 7.7 With the route forced to 500 the grid is non-interactive and shows "Nie udało się sprawdzić dostępności." on desktop and inside the mobile sheet (F3) (desktop 1440px: message shown, gridInert=true. Mobile 390px: sheet layer present, message inside it, grid inert inside it, retry inside it. Driving this corrected D19's stated rationale — the panel is `idle` until a COMPLETE range is picked, so on a freshly-opened modal it never states the failure on EITHER breakpoint (occurrences=1, inside the sheet, on both). The picker is not a duplicate for the covered case; it is the only surface that reports the failure at all)
- [x] 7.8 **Spróbuj ponownie** recovers the surface once the route is restored, and a failed re-read no longer discards good ranges (F7) (driven with the route forced to 500: retry present=true, and errorCleared=true after unrouting. Carry-over driven separately via a failed submit pre-flight: busyCells=3 preserved, and after the F1 review fix pointer=hit-testable rather than blocked. NOTE: the retry had to be added to the PICKER as well as the panel — the panel's error branch needs a complete range, and the grid is inert on a failed read, so a panel-only retry was unreachable in the one state that needs it)
...
- [x] 7.10 The pre-flight request carries `no-store` and is not served from cache on a repeated create attempt (F6) (request side: Chromium serializes fetch `cache:"no-store"` as cache-control: no-cache + pragma: no-cache on the wire — the observable marker that the option applied, the no-store semantics being enforced by an internal load flag. Response side, added by review finding F4: `cache-control: no-store` on the route itself, curl-verified, so intermediaries and future callers are covered too)
- [x] 7.11 A fully-blocked day's fill measures `#D7DCE3` with only its label at 75% opacity; the nav gap is contract-backed (F8) (measured on the 2026-08-27 cell: bg=rgb(215,220,227) = #D7DCE3 exactly, buttonOpacity=1, label color carrying the /0.75 alpha. Previously opacity-75 sat on the button and faded the fill with it to ≈#E1E5EA. Nav gap: the design source draws `<div style={{display:'flex', gap:6}}>` around the pair and Tailwind's gap-1.5 IS 6px — so the code was already right and the CONTRACT was silent; recorded there rather than changed here)
- [x] 7.12 Vision-diff against the six boards clean apart from recorded deviations, now including D19 (F3, F7) (ran 2026-08-21 against all six boards. App captured at the boards' own viewports and deviceScaleFactor 2, fontsReady=true (`document.fonts.check("650 13px Inter")`) and zero page errors on every shot. PUNCH-LIST EMPTY. Differences found, all pre-recorded: fixture data (vehicle/month/dates); D2 — no clashing-booking card in the conflict panel; the vehicle-thumbnail glyph (S-12 contract). Measured rather than eyeballed, twice catching a wrong visual read: half-cell orientation is correct (14=cell-busy-pm lower-right, 19=cell-busy-am upper-left) and the range middle IS square (22/24 radius 9px #B43638 weight 700; 23 radius 0px #FBE4E1 weight 500). The F8 fill now measures rgb(215,220,227)=#D7DCE3 with opacity 1 and the alpha on the label, matching the boards. NOT DIFFABLE — no board exists for the four states this phase added, so they are deviation-recorded only (D19): grid loading; grid error with no ranges; grid error WITH carried ranges (operable, added by review finding F1); and the retry action in both the panel and the picker)
- [x] 7.13 Stale references cleared: the hook's comment no longer cites the deleted `useAvailability`; the S-12 contract's available-state bullet no longer quotes the retired copy (F9) (`grep -rn useAvailability src/` → no matches; the citation fell out with the F4 rewrite. The S-12 bullet now states there is no subtext, defers to D10 as authoritative, and flags the retired strings so they are not reintroduced)
- [x] 7.14 Phase 6 §3's Files line records `src/components/hooks/useMediaQuery.ts` (F5) (listed with the reason it is a hook and not a responsive variant: the picker needs two different POSITIONS in the tree, which a Tailwind variant cannot express without mounting two live copies of an interactive widget)

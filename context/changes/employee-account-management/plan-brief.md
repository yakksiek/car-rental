...
**In scope:** service-role key + admin client; `list_staff` / `deactivate_staff` RPCs + `profiles` columns; `/api/staff*` admin mutations (invite / deactivate / reset); PKCE recovery flow (`forgot-password` → `callback` → `reset-password`) shared with invite-accept; the Employees page + admin-only "Zespół" nav; integration tests + Playwright e2e (auth flows via Inbucket + admin CRUD).

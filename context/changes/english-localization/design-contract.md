...
| 5   | No **focus-visible** or **hover** state specified for `LangToggle` or `ActionMenu`.                                                                                                                                                                                                                             | Medium           | We author both; not a blocker. ✅ **CLOSED 2026-09-05** — focus-visible shipped in Phase 3; hover was missed and landed in Phase 12, values recorded in each control's spec below. |
...
- `deviation(no hover state in source)` — likewise, and RECORDED HERE because the contract left it
  unspecified while §2 item 5 promised we would author it. Filled 2026-09-05 (impl-review F8), taking
  the values the neighbouring controls already use rather than inventing a treatment:

  | Tone  | Base                     | Hover                                                                                                   |
  | ----- | ------------------------ | ------------------------------------------------------------------------------------------------------- |
  | light | `#FFFFFF` (`bg-card`)    | `hover:bg-background` — the same swap `LangRow` and the `ActionMenu` panel rows use                     |
  | dark  | `rgba(255,255,255,0.14)` | `hover:bg-white/25` — one step up the translucent-white fill, shared with the dark `ActionMenu` trigger |

  Both animate through the `transition-colors` the control already carried, which until now had
...
**Trigger hover** — `deviation(no hover state in source)`, filled 2026-09-05 (impl-review F8). Same
rule as `LangToggle`: borrow the neighbours, do not invent.

| Tone  | Base                     | Hover                                                                                                                                        |
| ----- | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------- |
| light | `#0F172A` (`ink`)        | `hover:opacity-90` — a SOLID dark fill, so it follows the landing CTA's treatment rather than a background swap that would need a second ink |
| dark  | `rgba(255,255,255,0.15)` | `hover:bg-white/25` — the same step the dark `LangToggle` takes                                                                              |

`transition-colors` added to the trigger alongside it; it previously had none.

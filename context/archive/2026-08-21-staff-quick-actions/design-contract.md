...
| **D8**  | ~~`deviation(reach)`~~ → `exact` | Mobile Pulpit        | ~~The design omits the affordance on `ScreenWorkerDash` because `Dyspozytornia` (273px, unbreakable, at `fontSize: 40`) already clips its avatar at 390px. Our title is `Pulpit`, so the constraint does not transfer; we add the circle, with the 360px fit as a hard success criterion.~~ **Superseded 2026-08-26** — `ScreenWorkerDash` was re-pulled at the S-13 merge and now draws `<QuickAddButton mode="mobile" />` itself, between the magnifier and the avatar (44 / 40 / 44, gap 10). The design no longer omits the affordance, so the divergence is gone. Full write-up: `context/changes/staff-search-dashboard-only/design-contract.md` §A + Deviations. The 360px fit still holds (cluster 148px). |

...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "coordination overhead + data trapped on paper"
    - topic: "insight"
      decision: "small operators don't invest in software until pain is acute; existing tools target B2C car rental, not B2B commercial fleet"
    - topic: "persona scope"
      decision: "employee and owner/admin are co-primary personas"
    - topic: "auth model"
      decision: "mixed — public fleet browsing, login (email+password) required to book; employees and admins always log in"
    - topic: "role split"
      decision: "three roles but no vehicle-to-employee assignment; employees manage fleet; admin's unique power is employee management"
    - topic: "mvp scope"
      decision: "kept photo upload, digital signature, auto-email; dropped customer dashboard/account/invoices for v1"
    - topic: "timeline"
      decision: "3 weeks, day job work, no hard deadline"
    - topic: "late fees"
      decision: "flag-only in v1 — no automatic late-fee calculation; employee handles charges manually"
    - topic: "product type"
      decision: "web app"
    - topic: "target scale"
      decision: "medium (dozens to a hundred users); single-tenant, one deployment per company"
    - topic: "timing"
      decision: "no hard deadline; day job work; 3-week MVP"
  frs_drafted: 14
...
Role: Operational staff at a local commercial vehicle rental company. Manages a set of assigned vehicles day-to-day — processes reservations, performs vehicle handovers (issue and return), documents mileage/fuel/damage on paper, tracks overdue returns manually.
...
Role: Business owner or manager of the rental operation. Oversees the entire fleet, all employees, and all reservations. Responsible for pricing, fleet composition, and employee assignment.
...
Two authenticated roles (employee, admin) + unauthenticated customer flow. Authentication via email + password.
...
- **Employee**: Logs in with access to all vehicles (no per-employee assignment). Can manage fleet (add/edit/remove vehicles), accept/reject reservations, fill out handover protocols (issue and return with photos and digital signature), view overdue returns on their dashboard.
- **Admin**: Everything an employee can do, plus employee management (add/remove staff accounts). Full fleet-wide oversight.

Note: Customer accounts (login, history, statuses) are deferred to v2. In v1, customers interact only via the public site and receive protocols by email.
...
- The return protocol pre-fills data from the issue protocol and highlights deltas (mileage difference, fuel level change, new damage).
...
### MVP flow (scoped down for v1)
1. Customer lands on the site (no login)
2. Chooses vehicle category, browses/filters available vehicles
3. Picks vehicle + dates, submits reservation request (name, email, phone)
4. Employee sees the request, accepts or rejects it
5. Pickup: employee fills issue protocol on device (mileage, fuel, damage, photos, signature)
6. Protocol auto-emailed to customer
7. Return: employee opens return protocol — pre-filled from issue (mileage comparison, fuel delta, new damage, photos, signature)
8. Employee dashboard: assigned vehicles, reservations, overdue returns
9. Admin: full fleet overview, employee management, all reservations

### Deferred to v2
- Customer accounts (login, dashboard, history, statuses)
- Auto-account creation from protocol data
...
- **Then** the protocol is saved and auto-emailed to the customer; when the vehicle returns, the return protocol opens pre-filled from the issue data with deltas highlighted

#### Acceptance Criteria
- Issue protocol captures: mileage, fuel level, damage notes, photos, digital signature
- Return protocol pre-fills issue data and shows comparison (km driven, fuel difference, new damage)
...
- Protocol forms (issue and return) must be fully usable on mobile phones and tablets — employees fill these on-site at the vehicle, using the device camera for photos and touch input for digital signature.
- All user-facing pages load within 2 seconds on a standard broadband connection.

## Non-Goals

- **No online payments.** Reservation + payment at vehicle pickup. No Stripe/PayPal/billing integration in v1. Keeps financial complexity out.
...
- **No native mobile app.** Responsive web only. Employees use their phone/tablet browsers for protocols.
...
- **No multi-tenancy.** Single-company deployment. No SaaS platform for multiple rental operators.
...
1. ~~**Audit trail for fleet changes.**~~ Resolved: no audit trail in v1. Deletion guard (OQ-2) reduces the risk. Defer to v2.
...
3. ~~**Password reset for employee accounts.**~~ Resolved: employees can self-service reset their password via email. Admin retains account create/remove control. No friction between the two.

## Quality cross-check

All 6 greenfield quality elements present. No gaps. Status: accepted.

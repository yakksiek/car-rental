...
- **Problem**: PNG screenshots are vision tokens and cost far more context than text. When the plan only references screen _numbers_, /10x-implement re-opens the PNG for every slice (and on every resume), burning the image cost repeatedly. The image is already on screen during /10x-plan — that's the cheapest moment to convert it.

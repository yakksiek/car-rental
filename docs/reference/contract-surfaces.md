...
| `StaffMember`, `CreateEmployeeResult` (arms: `created` / `reactivated` (carries the activation-mail outcome) / `duplicate_active` / `provision_rolled_back` / `provision_orphaned` / `unauthorized`), `DeactivateResult`, `employeeInviteSchema` | `src/lib/services/staff.ts`                                                                                      | staff API routes + `StaffList` island                             |

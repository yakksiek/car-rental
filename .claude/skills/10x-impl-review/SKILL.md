...
3. **Design fidelity (UI changes only)** — if the change touches a user-facing surface (a page, component, layout, or stylesheet — per the project's rules) and the change folder has a `design-contract.md`, statically compare the shipped markup/styles against it. Flag: a dimension that doesn't match the contract's exact value; a **loose or magic value** (an eyeballed px/rem, or a range like "15–20px" shipped as an in-between guess) where the contract gives an exact one; a value invented with no basis in the contract or an existing design token. A range in the code is acceptable only where the contract marks it an intentional/responsive `deviation(reason)`. Do not run a rendered vision-diff here — that belongs to `/10x-implement` and the plan's design-audit; this is a static read of values against the contract.

4. **Budget pattern work to scope** — if the diff changed ≤3 files, spend minimal time on patterns (not much to compare). Scale pattern depth with change scope.
